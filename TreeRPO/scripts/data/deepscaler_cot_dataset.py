    ...
    def process_fn(example: Dict[str, Any], idx: int, instruction: str = None) -> Optional[Dict[str, Any]]:
        ...
        data = {
            ...
            "prompt": [
                {
                    "role": "system",
                    "content": "You are a math assistant. Please solve the given question."
                },
                {
                "role": "user",
...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_dir', default=os.path.expanduser('/hpc2hdd/home/zyang398/yangzhch6/projs/TreeRPO/TreeRPO-v4/data'),
    ...
    # Initialize datasets
    train_datasets = [TrainDataset.Math.DEEPSCALER_COT]
    ...
    process_fn = make_map_fn(split='train', data_source=train_datasets[0].value)
    ...
    train_df.to_parquet(os.path.join(local_dir, 'train/deepscaler_cot_train.parquet'))
    test_df = pd.DataFrame(test_data)
    test_df.to_parquet(os.path.join(local_dir, 'test/deepscaler_cot_test.parquet'))
    ...
    for test_dataset, test_data_list in zip(test_datasets, test_datasets_data):
        ...
        process_fn = make_map_fn(split='test', data_source=test_dataset.value)
        ...
        test_df = pd.DataFrame(test_data)
        test_df.to_parquet(os.path.join(local_dir, f'test/{dataset_name}.parquet'))
