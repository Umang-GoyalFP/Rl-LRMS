...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto,  **kwargs) -> DataProto:
        ...
        if prompts.meta_info.get('val_temperature', None):
            ...
            is_validation = True
        if prompts.meta_info.get('max_gen_tokens', None):
            kwargs['max_tokens'] = prompts.meta_info['max_gen_tokens']
        
        original_config_n = self.config.n
        if prompts.meta_info.get('sample_n', None):
            self.config.n = prompts.meta_info['sample_n']
        ...
        if prompts.meta_info.get('max_gen_tokens', None):
            pad_max_length = prompts.meta_info['max_gen_tokens']
        else:
            pad_max_length = self.sampling_params.max_tokens

        # Pad sequences if needed
        if response.shape[1] < pad_max_length:
            response = pad_sequence_to_length(
                response, pad_max_length, self.pad_token_id)
            log_probs = pad_sequence_to_length(
                log_probs, pad_max_length, self.pad_token_id)
        ...
        self.config.n = original_config_n
